...
AIFDE 工程师（AI 赋能体检）是一个面向企业老板/个人老板的 AI 机会检测 skill：通过对话式访谈找到业务里 3~5 个具体任务，逐个评估**值不值得做（Opportunity）、能不能做（Feasibility）、有没有风险（Risk）、划不划算（Economics）**，输出任务级结论（优先试点/值得验证/先补基础/仅限辅助/暂不投入）与企业四档总览，并生成深色科技风 HTML 体检报告（含就绪度雷达图、词云、PoC 最小验证方案）。
